...
import { updateUserLocation } from "@/lib/services/location.service";
    ...
    const handleGeolocate = async () => {
        ...
        if (navigator.permissions) {
            try {
                ...
                if (permissionStatus.state === 'denied') {
                    alert("Location access is blocked. Please enable location permissions in your browser settings and reload the page.");
                    return;
                }
            } catch (e) {
                console.log(e)
        ...
        navigator.geolocation.getCurrentPosition(
            async (position) => {
                ...
                if (accessToken) {
                    try {
                        await updateUserLocation({ latitude, longitude, accessToken });
                    } catch (error) {
                        ...
                        (retryError) => {
                            ...
                            switch (retryError.code) {
                                ...
                                case retryError.POSITION_UNAVAILABLE:
                                    message += "Your device's location service is unavailable. Please ensure GPS/Location Services are enabled in your device settings.";
                                    break;
                                case retryError.TIMEOUT:
                                    message += "Location request timed out. Please ensure you have a clear view of the sky (for GPS) or are connected to WiFi.";
                                    break;
                                default:
        ...
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-[calc(100vw-2rem)] sm:max-w-[500px] max-h-[90vh] overflow-y-auto rounded-2xl">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Customize your search to find the perfect stalls for your taste and
                        budget.
                    </DialogDescription>
                </DialogHeader>
