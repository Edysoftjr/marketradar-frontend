...
const AddToCartSheet = ({
    ...
    const selectedSizeObj = sizes.find(
        (size) => size.id && size.id.toString() === selectedSize
    );

    const selectedAddonObjs = addons.filter(
        (addon) => addon.id && selectedAddons.includes(addon.id.toString())
    );


    const basePrice = selectedSizeObj?.price || 0;
    const addonsPrice = selectedAddonObjs.reduce(
    ...
    return (
        <Sheet open={open} onOpenChange={onOpenChange}>
            <SheetContent
                side="bottom"
                className="h-[90vh] rounded-t-3xl border-t-0 p-0 overflow-hidden"
            >
